...
public class HeadmanHandler extends ChainHandler {

    @Override
    public void doHandler(double score) {
        Log.i(RESP_CHAIN_TAG, "传递到了：组长");
        if (score >= 60 && score < 70){
            Log.i(RESP_CHAIN_TAG, "审核人：组长；自评分数：" + score + "分；审核结果：通过");
        } else {
            if (null != getNextHandler()){
                Log.i(RESP_CHAIN_TAG, "不属于组长的职责，继续传递。。。");
                getNextHandler().doHandler(score);
            } else {
                Log.i(RESP_CHAIN_TAG, "组长没有处理，无下一个处理人，终止传递。。。");
            }
        }
    }
}
