...
public class HeadOffice extends Company {
    ...
    public HeadOffice(String name) {
        super(name);
        this.name = name;
        Log.i(COMPOSITE_TAG, "创建总公司：" + name);
    }

    @Override
    public void add(Company company) {
        subCompanies.add(company);
        Log.i(COMPOSITE_TAG, name + "新增了一个分公司：" + company.getName());
    }
    ...
    public void addCompanies(List<Company> companies) {
        ...
        Log.i(COMPOSITE_TAG, name + "批量新增了" + companies.size() + "个分公司");
    }

    @Override
    public void removeCompanies(List<Company> companies) {
        subCompanies.removeAll(companies);
        Log.i(COMPOSITE_TAG, name + "批量解除了" + companies.size() + "个分公司");
    }
}
